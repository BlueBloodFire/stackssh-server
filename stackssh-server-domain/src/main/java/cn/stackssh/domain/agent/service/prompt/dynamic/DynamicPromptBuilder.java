...
public class DynamicPromptBuilder {
    ...
    public String buildMessagePrefix(PromptContextVO ctx) {
        ...
        // Phase 4: 意图增强 — 服务状态、配置文件、日志
        SearchContext sc = ctx.getSearchContext();
        if (sc != null && !sc.isEmpty()) {
            if (!sc.getServiceStatus().isEmpty()) {
                sb.append("\n[服务状态]\n");
                for (Map.Entry<String, String> e : sc.getServiceStatus().entrySet()) {
                    sb.append(e.getKey()).append(": ").append(e.getValue()).append("\n");
                }
                hasContent = true;
            }
            if (!sc.getFileContents().isEmpty()) {
                sb.append("\n[相关配置文件]\n");
                for (Map.Entry<String, String> e : sc.getFileContents().entrySet()) {
                    sb.append("# ").append(e.getKey()).append("\n").append(e.getValue()).append("\n");
                }
                hasContent = true;
            }
            if (!sc.getRecentLogs().isEmpty()) {
                sb.append("\n[最近日志]\n");
                for (Map.Entry<String, String> e : sc.getRecentLogs().entrySet()) {
                    sb.append("## ").append(e.getKey()).append("\n").append(e.getValue()).append("\n");
                }
                hasContent = true;
            }
