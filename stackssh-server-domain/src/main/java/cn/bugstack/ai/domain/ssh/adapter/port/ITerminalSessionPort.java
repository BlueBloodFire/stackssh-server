...
public interface ITerminalSessionPort {
    ...
    /**
     * 停止录制并返回所有录制事件（offsetMs, data），清空缓冲
     *
     * @param sessionId 会话ID
     * @return 录制事件列表；若无录制则返回空列表
     */
    java.util.List<cn.bugstack.ai.domain.ssh.model.entity.TerminalRecordingEntity.Event> stopRecording(String sessionId);
