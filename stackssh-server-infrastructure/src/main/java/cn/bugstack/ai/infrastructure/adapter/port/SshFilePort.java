...
public class SshFilePort implements ISshFilePort {
    ...
    private Session requireSession(String connectionId) {
        ...
        // 主动发 keep-alive 探测 Session 是否真实存活（防止 TCP 已断但 isConnected() 仍为 true）
        try {
            session.sendKeepAliveMsg();
        } catch (Exception e) {
            log.warn("SSH Session keep-alive 失败，连接已失效 connectionId={} reason={}", connectionId, e.getMessage());
            closeSftp(connectionId);
            try { sshSessionPort.disconnect(connectionId); } catch (Exception ignored) {}
            throw new IllegalStateException("SSH连接已失效，请重新连接");
        }
        // SFTP channel 失效时提前清理，让 getOrOpenSftp 重建
    ...
    private synchronized ChannelSftp getOrOpenSftp(String connectionId, Session session) {
        ...
        try {
            ...
            // "channel is not opened" 表示底层 TCP 已断但 isConnected() 未感知（空闲超时/防火墙）
            // 主动清理失效 Session，让前端重新连接
            boolean staleSession = e.getMessage() != null &&
                    (e.getMessage().contains("channel is not opened") || e.getMessage().contains("session is down"));
            if (staleSession) {
                log.warn("检测到失效 SSH Session，清理缓存 connectionId={}", connectionId);
                try { sshSessionPort.disconnect(connectionId); } catch (Exception ignored) {}
                throw new IllegalStateException("SSH连接已失效，请重新连接", e);
            }
