...
public class TerminalSessionPort implements ITerminalSessionPort {
    ...
    /** connectionId -> 该连接下所有活跃 sessionId 集合（支持多标签） */
    private final Map<String, java.util.Set<String>> connectionSessions = new ConcurrentHashMap<>();

    /** sessionId -> 录制开始时间戳（ms），null 表示未录制 */
    private final Map<String, Long> recordingStartTime = new ConcurrentHashMap<>();

    /** sessionId -> 录制事件列表 */
    private final Map<String, List<TerminalRecordingEntity.Event>> recordingBuffers = new ConcurrentHashMap<>();
    ...
    public String openTerminal(String connectionId, int cols, int rows) {
        ...
        try {
            ...
            connectionSessions.computeIfAbsent(connectionId, k -> java.util.Collections.newSetFromMap(new ConcurrentHashMap<>())).add(sessionId);
            ...
            try {
                ...
                while (true) {
                    try {
                        ...
                        // 如果开启了录制，写入录制缓冲区
                        Long recStart = recordingStartTime.get(sessionId);
                        List<TerminalRecordingEntity.Event> recBuffer = recordingBuffers.get(sessionId);
                        if (recStart != null && recBuffer != null) {
                            long offsetMs = System.currentTimeMillis() - recStart;
                            // data 使用 Base64 编码保留控制字符
                            String encoded = java.util.Base64.getEncoder().encodeToString(
                                    text.getBytes(StandardCharsets.UTF_8));
                            recBuffer.add(TerminalRecordingEntity.Event.builder()
                                    .offsetMs(offsetMs)
                                    .data(encoded)
                                    .build());
                        }
    ...
    public void startRecording(String sessionId) {
        recordingStartTime.put(sessionId, System.currentTimeMillis());
        recordingBuffers.put(sessionId, new CopyOnWriteArrayList<>());
        log.info("终端录制已开启 sessionId={}", sessionId);
    }

    @Override
    public List<TerminalRecordingEntity.Event> stopRecording(String sessionId) {
        recordingStartTime.remove(sessionId);
        List<TerminalRecordingEntity.Event> events = recordingBuffers.remove(sessionId);
        log.info("终端录制已停止 sessionId={} events={}", sessionId, events == null ? 0 : events.size());
    ...
    private void cleanup(String sessionId) {
        // 从 connectionSessions 集合中移除此 sessionId
        connectionSessions.values().forEach(set -> set.remove(sessionId));
        ...
        recordingStartTime.remove(sessionId);
        recordingBuffers.remove(sessionId);
    }
